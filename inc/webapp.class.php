...
//require(__ROOT__."/inc/attachementmgr.class.php");
...
class WebApp implements WebAppInterface
{
	...
	var $isdbg = 0; # Gconf::get('is_debug');
    ...
    */
    ...
    //- remedy by wadelau on Mon Nov 11 09:29:47 CST 2013
    # support batchly set
	function set($field,$value=null){
        ...
        if($value === null){
			...
			if(is_array($field)){
				...
				$this->hmf[$field] = ''; # remedy 	Tue Jun 24 20:53:23 CST 2014
			}
        }
        else{
		    $this->hmf[$field] = $value;
	...
	*/
	...
	//add new record
	//all new name must be start with "add"
	function add($newInfo) {
		...
		$hm = $this->setBy($fieldlist_str, null);

		//var_dump($hm);
		

		if ( $hm[0] != 1 ) { 	  
			$result = 0;  //Modify FAILED
			return $result;
		} else {
			$result = 1;  //Modify SUCCESSFULLY
		}
		
		$attachementMgr = new AttachementMgr();
		$attachements = $attachementMgr->addAttachements($this->getTbl(), $hm[1]['insertid']);		
	...
	//edit information
	//all new name must be start with "edit"
    #!!! deprecated below this LINE !!!
	function edit($newInfo) { 
		foreach($newInfo as $key=>$value)
		{
			$pos = strpos($key, 'edit');
            $hasKey = 1;
		...
		$attachementMgr = new AttachementMgr();
		$attachements = $attachementMgr->addAttachements($this->getTbl(), $newInfo['edit_id']);	
